...
export default function DecodeVideo() {
  ...
  const handleDecode = async () => {
    if (!video || !secretCode) {
      alert("Please upload a video and enter the secret code.");
      return;
    }
    ...
    try {
      const response = await axios.post(
        "http://127.0.0.1:5000/decode_video",
        formData
      );
      ...
      console.error("Error decoding the video:", error);
      alert(
        error.response?.data?.error ||
          "Failed to decode the video. Please try again."
      );
